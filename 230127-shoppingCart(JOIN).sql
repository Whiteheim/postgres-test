...
-- 상품 #1 돈까스의 총판매금액은 51,600원이다. 이를 쿼리를 통해 조회하시오.
select
...
-- 쿠마가 구매한 모든 상품 목록 중, 상품명과 가격, 주문수량 조회
select 
